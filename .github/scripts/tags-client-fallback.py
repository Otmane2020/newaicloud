...
path = Path('src/components/seo/MinimalMetadataWorkspaces.tsx')
text = path.read_text()

marker = 'export function MinimalTagsWorkspace() {'
helpers = r'''function parseProductTags(value: unknown): string[] {
...
if helpers.strip() not in text:
    if marker not in text:
        raise SystemExit('MinimalTagsWorkspace marker not found')
    text = text.replace(marker, helpers + marker, 1)

old_select = '.select("id,title,tags,vendor,image_url,product_type,optimization_count,seo_synced_to_shopify")'
new_select = '.select("id,title,tags,vendor,image_url,product_type,category,sub_category,ai_color,ai_material,optimization_count,seo_synced_to_shopify")'
if old_select in text:
    text = text.replace(old_select, new_select, 1)

start = '  const generate = async (ids: string[]) => {'
workspace_index = text.find(marker)
start_index = text.find(start, workspace_index)
if start_index == -1:
    raise SystemExit('Tags generate function start not found')
end_marker = '  const tagArray = (value: string | null) =>'
end_index = text.find(end_marker, start_index)
if end_index == -1:
    raise SystemExit('Tags generate function end marker not found')

new_generate = r'''  const generate = async (ids: string[]) => {
...
text = text[:start_index] + new_generate + text[end_index:]
