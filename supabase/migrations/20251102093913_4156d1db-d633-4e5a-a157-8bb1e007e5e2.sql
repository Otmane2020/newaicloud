-- Fix check_optimization_allowed to properly respect force parameter
...
BEGIN
  ...
  -- If force is true, allow re-optimization for paid users
  IF p_force AND v_is_paid THEN
    -- Still check monthly limits for paid users
    ...
    IF v_usage_count >= v_max_optimizations THEN
      RETURN jsonb_build_object(
        ...
        'message', 'Limite mensuelle d''optimisations atteinte. Passez à un plan supérieur.',
        'usage', v_usage_count,
        'limit', v_max_optimizations
      );
    END IF;
    
    -- Force re-optimization allowed
    RETURN jsonb_build_object(
      'allowed', TRUE,
      'is_trial', FALSE,
      'is_paid', TRUE,
  ...
  -- Trial users: max 1 optimization per resource (force ignored for trial)
  IF v_is_trial THEN
  ...
  ELSIF v_is_paid THEN
    -- Paid users without force: check if already optimized
    IF v_optimization_count >= 1 AND NOT p_force THEN
      RETURN jsonb_build_object(
        'allowed', FALSE,
        'reason', 'already_optimized',
        'message', 'Ce contenu a déjà été optimisé. Utilisez "Forcer l''optimisation" pour ré-optimiser.',
        'count', v_optimization_count,
        'last_optimized_at', v_last_optimization
      );
    END IF;
