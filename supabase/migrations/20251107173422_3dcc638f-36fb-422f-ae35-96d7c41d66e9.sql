-- Amélioration de increment_usage pour plafonner TOUTES les limites incluant products et stores
...
BEGIN
  ...
  IF p_field = 'products_count' THEN
    IF v_is_trial THEN
      v_max_limit := 10; -- Trial: 10 produits max
    ELSE
      SELECT COALESCE(sp.max_products, 100) INTO v_max_limit
      FROM profiles p
  ...
  ELSIF p_field = 'shopify_stores_count' THEN
    IF v_is_trial THEN
      v_max_limit := 1; -- Trial: 1 store max
    ELSE
      SELECT COALESCE(sp.max_shopify_stores, 5) INTO v_max_limit
      FROM profiles p
      LEFT JOIN subscription_plans sp ON p.current_plan_id = sp.id
      WHERE p.id = p_seller_id;
    END IF;
  ELSIF p_field = 'optimizations_count' THEN
    ...
    ELSE
      SELECT COALESCE(sp.max_optimizations_monthly, 100) INTO v_max_limit
      FROM profiles p
      LEFT JOIN subscription_plans sp ON p.current_plan_id = sp.id
      WHERE p.id = p_seller_id;
    END IF;
