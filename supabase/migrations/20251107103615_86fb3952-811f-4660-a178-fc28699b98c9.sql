-- Fix: Decrement shopify_stores_count for current AND future months when disconnecting
...
BEGIN
  ...
  -- CRITICAL FIX: Update usage counters for current month AND all future months
  ...
  SET 
    ...
    updated_at = now()
  WHERE seller_id = OLD.user_id 
    AND month >= DATE_TRUNC('month', CURRENT_DATE);  -- >= instead of = to include future months
