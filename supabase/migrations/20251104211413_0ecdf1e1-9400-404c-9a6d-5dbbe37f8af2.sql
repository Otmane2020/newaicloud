-- Modifier la fonction check_optimization_allowed pour permettre aux utilisateurs payants de ré-optimiser
CREATE OR REPLACE FUNCTION public.check_optimization_allowed(p_user_id uuid, p_resource_type text, p_resource_id uuid, p_force boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSIF v_is_paid THEN
    -- Paid users: check ONLY monthly limits (remove already_optimized check)
    ...
    -- For paid users, allow re-optimization as long as monthly quota is available
    -- (removed the already_optimized check)
  ELSE
