...
serve(async (req) => {
  ...
  try {
    const { prompt, article_id, collection_id, type } = await req.json();
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(
          JSON.stringify({ error: "Crédit insuffisant. Veuillez ajouter des fonds à votre workspace Lovable AI." }),
