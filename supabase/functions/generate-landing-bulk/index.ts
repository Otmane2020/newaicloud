...
serve(async (req) => {
  ...
  try {
    ...
    // Clean old brand names from description to prevent AI confusion
    const cleanDescriptionBrands = (description: string, newVendor: string): string => {
      if (!description || !newVendor) return description || "";
      
      // Common brand patterns to detect and replace in description
      // Matches: "par Velvet", "by Nike", "de DIANA", "from Brand"
      let cleaned = description;
      
      // Replace "par/by/de/from [BrandName]" patterns
      cleaned = cleaned.replace(/\b(par|by|de|from)\s+([A-Z][a-zA-Z]+|[A-Z]{2,})\b/gi, `$1 ${newVendor}`);
      
      // Replace standalone UPPERCASE brand names (like "DIANA", "VELVET") but preserve common words
      const commonWords = ['LED', 'USB', 'XXL', 'XXS', 'XL', 'CM', 'MM', 'EUR', 'USD', 'TV', 'HD', 'DVD', 'CD', 'PC', 'OK'];
    ...
    // Clean the description to remove old brand references
    const cleanedDescription = vendor 
      ? cleanDescriptionBrands(productDescription || "", vendor)
      : productDescription;
...
═══════════════════════════════════════════════════════
🚨 RÈGLE CRITIQUE - NOM DE MARQUE (TRÈS IMPORTANT!)
═══════════════════════════════════════════════════════
⚠️ ATTENTION: La description peut contenir d'ANCIENS noms de marque.
❌ Tu dois IGNORER COMPLÈTEMENT tout nom de marque présent dans la description !
✅ Tu dois utiliser UNIQUEMENT ce nom de marque: "${vendor || "Marque Premium"}"
✅ Dans TOUT le contenu généré, utilise EXCLUSIVEMENT: "${vendor || "Marque Premium"}"
✅ Exemples corrects: "chaises ${vendor}", "la collection ${vendor}", "par ${vendor}"
❌ Ne JAMAIS copier les noms de marque de la description (DIANA, Velvet, etc.)

═══════════════════════════════════════════════════════
📦 INFORMATIONS PRODUIT
═══════════════════════════════════════════════════════
TITRE: ${productTitle}
DESCRIPTION (ignorer les noms de marque ici!): ${cleanedDescription || "Produit de qualité supérieure, conçu avec soin pour répondre aux exigences les plus élevées."}
MARQUE À UTILISER PARTOUT: ${vendor || "Marque Premium"} ← SEUL nom autorisé!
${highlightsSection}
...
⚡ CONSIGNES FINALES
═══════════════════════════════════════════════════════
- MARQUE: UNIQUEMENT "${vendor || "Marque Premium"}" - JAMAIS d'autres noms de la description!
