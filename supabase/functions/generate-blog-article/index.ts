...
async function generateSingleArticle(requestData: any, supabaseClient: any, apiKey: string, authHeader?: string) {
  try {
      ...
      return products
        .map((product) => {
          ...
          if (images.length === 0 || images.length === 1) {
            ...
            galleryHtml = `<div class="product-gallery-grid">${images
              .slice(0, 4)
              .map(
                (img: any) => `
            <img src="${img.src}" alt="${img.alt || product.title}" class="gallery-image" loading="lazy">
    ...
    const layoutConfig: Record<
      ...
      { tocColumns: number; productColumns: number; maxWidth: string }
    > = {
        ...
        messages: [
          {
            role: "system",
            content:
              "Expert en rédaction e-commerce. Tu génères UNIQUEMENT du contenu textuel en JSON, sans toucher au HTML.",
          },
...
<body>
  <article>
    ...
    if (remainingPlaceholders && remainingPlaceholders.length > 0) {
      console.warn(
        `⚠️ [VALIDATION] ${remainingPlaceholders.length} placeholders non remplacés:`,
        remainingPlaceholders,
      );

      // Nettoyer les placeholders restants
      content = content.replace(/\[contenu.*?\]/gi, "").replace(/\[description.*?\]/gi, "");
      ...
      .insert([
        {
          ...
          meta_description:
            ...
            `Guide complet : ${optimizedTitle}. Comparatif expert, conseils d'achat et sélection des meilleurs produits. Livraison offerte.`,
