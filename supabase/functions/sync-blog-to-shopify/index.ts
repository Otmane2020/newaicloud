...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!article_ids || !Array.isArray(article_ids) || article_ids.length === 0) {
      return new Response(JSON.stringify({ error: "Missing or invalid article_ids array" }), {
        status: 400,
    ...
    for (const articleId of article_ids) {
      try {
        ...
        if (!article) {
          results.errors++;
          results.details.push({ articleId, error: "Article not found" });
          continue;
        }
        ...
        if (!store) {
          results.errors++;
          results.details.push({ articleId, error: "No Shopify connection found" });
          continue;
        }
        ...
        results.success++;
        results.details.push({ articleId, shopifyArticleId: created.article?.id });
