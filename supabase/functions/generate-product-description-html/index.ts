...
// ✅ CORRECTION 1: Helper pour éviter "[object Object]"
...
// ✅ CORRECTION 4: Timeout manuel pour éviter les hangs
...
serve(async (req) => {
  ...
  // ✅ CORRECTION 3: Fallback pour userPreferences / options
  const userOptions = body.options || body.userPreferences || {};
  const {
    layout,
    designStyle,
    contentLength,
  ...
  const userPreferences = Object.keys(userOptions).length > 0 ? {
    layout,
    designStyle,
    contentLength,
    colorScheme,
...
${visionAnalysis ? `Internal Visual Analysis (DO NOT DISPLAY TO CUSTOMER - USE ONLY FOR CONTENT ENRICHMENT): ${typeof visionAnalysis === "string" ? visionAnalysis : safeJson(visionAnalysis)}` : ""}
${dimensions ? `Dimensions: ${typeof dimensions === "string" ? dimensions : safeJson(dimensions)}` : ""}
...
`;
    ...
    // ✅ CORRECTION 5: Nettoyer le prompt avant l'appel AI (éviter 400 Gemini)
    ...
    // ✅ CORRECTION 6: Sanitize qui ne supprime pas <style>
    content = content
      .replace(/```json/g, "")
      .replace(/```html/g, "")
      .replace(/```/g, "")
