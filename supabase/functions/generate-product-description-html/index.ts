...
serve(async (req) => {
  ...
  try {
    const { 
      ...
      customHighlights,
      language = "fr",
      provider = "gemini" // 'lovable' ou 'gemini'
    } = await req.json();
    ...
    console.log("🧠 Generating product landing page for:", title, "with provider:", provider);

    // Vérifier les API keys selon le provider
    ...
    if (enrichedAttributes) {
      enrichedContext += `\n🎨 Visual Attributes: ${JSON.stringify(enrichedAttributes.visual || {})}`;
      enrichedContext += `\n📐 Dimensions: ${JSON.stringify(enrichedAttributes.dimensions || {})}`;
      enrichedContext += `\n🏷️ Categorization: ${JSON.stringify(enrichedAttributes.categorization || {})}`;
    }
    if (variants?.length) {
      enrichedContext += `\n🔀 Variants (${variants.length}): ${variants.map((v: any) => v.title).join(", ")}`;
    }
    if (vendor) {
      enrichedContext += `\n🏪 Brand/Vendor: ${vendor}`;
    }
...
`;
    ...
    if (provider === "lovable") {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Lovable AI generation failed: ${response.status}`);
      }

      const data = await response.json();
      content = data.choices?.[0]?.message?.content?.trim();
      
      if (!content) throw new Error("No content generated by Lovable AI");
    } else {
