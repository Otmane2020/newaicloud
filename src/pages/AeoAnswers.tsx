...
import { 
  ...
  Sparkles, FileText, RefreshCw, Filter
} from "lucide-react";
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-slate-900/50 border-violet-500/20 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-white">{answers.filter(a => a.article_id).length}</p>
              <p className="text-xs text-white/50">{language === 'fr' ? "Avec article" : "With article"}</p>
            </div>
          </div>
        </Card>
        <Card className="bg-slate-900/50 border-violet-500/20 p-4">
            ...
            <Card key={answer.id} className="bg-slate-900/50 border-violet-500/20 p-6 hover:border-violet-500/40 transition-all">
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                <div className="flex-1 space-y-3">
                  ...
                  <div className="pl-13 bg-slate-800/50 rounded-lg p-4 ml-13">
