    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[calc(100dvh-0.5rem)] w-[calc(100vw-0.5rem)] max-w-6xl flex-col overflow-hidden rounded-2xl p-0 sm:h-[94dvh] sm:w-[calc(100vw-2rem)] sm:rounded-3xl">
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto bg-slate-50/70 px-3 py-4 sm:px-6 sm:py-6">
                    ...
                    return (
                      ...
                      >
                        <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-slate-50 to-white">
                          ...
                          {selected && (
                            <span className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-full bg-violet-600 text-white shadow-lg"><Check className="h-4 w-4" /></span>
                          )}
                        </div>
              ...
              <div className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:grid-cols-[180px_minmax(0,1fr)] sm:p-4">
                ...
                <div className="flex min-w-0 items-center gap-3">
                  <div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50 sm:h-20 sm:w-20">
                    {selectedProduct?.image ? <img src={selectedProduct.image} alt="" className="h-full w-full object-contain p-1.5" /> : <ImageIcon className="h-5 w-5 text-slate-300" />}
                  </div>
                  <div className="min-w-0">
              ...
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,.85fr)]">
                <div className="space-y-4">
                  <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div><h3 className="font-bold text-slate-950">Creative direction</h3><p className="mt-0.5 text-xs leading-5 text-slate-500">Choose how strongly the ad should focus on the product versus its selling points.</p></div>
                    </div>
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      <button type="button" onClick={() => onGenerationModeChange("showcase")} className={cn("rounded-2xl border p-4 text-left transition", generationMode === "showcase" ? "border-violet-500 bg-violet-50 ring-2 ring-violet-100" : "border-slate-200 hover:border-violet-200")}>
                        <div className="flex items-center justify-between gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-white text-violet-700 shadow-sm"><Eye className="h-4 w-4" /></span>{generationMode === "showcase" && <Check className="h-4 w-4 text-violet-600" />}</div>
                        <p className="mt-3 text-sm font-bold text-slate-950">Showcase</p><p className="mt-1 text-xs leading-5 text-slate-500">Hero-product composition with clean hierarchy and premium commercial styling.</p>
                      </button>
                      <button type="button" onClick={() => onGenerationModeChange("strengths")} className={cn("rounded-2xl border p-4 text-left transition", generationMode === "strengths" ? "border-violet-500 bg-violet-50 ring-2 ring-violet-100" : "border-slate-200 hover:border-violet-200")}>
                        <div className="flex items-center justify-between gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-white text-violet-700 shadow-sm"><Star className="h-4 w-4" /></span>{generationMode === "strengths" && <Check className="h-4 w-4 text-violet-600" />}</div>
                        <p className="mt-3 text-sm font-bold text-slate-950">Strengths</p><p className="mt-1 text-xs leading-5 text-slate-500">Emphasize key benefits, features and a stronger promotional message.</p>
                      </button>
                    </div>
                  </section>
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div><h3 className="font-bold text-slate-950">Offer & on-image copy</h3><p className="mt-0.5 text-xs leading-5 text-slate-500">Only these fields influence the generated visual.</p></div>
                    </div>
                    <div className="mt-4 flex items-center justify-between gap-4 rounded-xl border border-slate-200 bg-slate-50 p-3.5">
                      <div><p className="text-sm font-semibold text-slate-900">Show product price</p><p className="mt-0.5 text-xs text-slate-500">Display the Shopify price in the visual when appropriate.</p></div>
                      <Switch checked={showPrice} onCheckedChange={onShowPriceChange} />
                    </div>
                    <div className="mt-4 space-y-2">
                      <div className="flex items-center justify-between gap-3"><label className="text-sm font-semibold text-slate-900">Tagline</label><span className="text-[11px] text-slate-400">Optional</span></div>
                      <Textarea value={tagline} onChange={(event) => onTaglineChange(event.target.value)} placeholder="e.g. A refined centerpiece for modern interiors" rows={3} className="resize-none rounded-xl border-slate-200 bg-white" />
                    </div>
                  </section>
                </div>
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 xl:self-start">
                  <div className="flex items-start gap-3">
                    ...
                    <div><h3 className="font-bold text-slate-950">Publishing setup</h3><p className="mt-0.5 text-xs leading-5 text-slate-500">Prepare the social post now, or leave it empty and download the creative only.</p></div>
                  </div>
                  <div className="mt-4 space-y-2">
                    <label className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Platform</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button type="button" onClick={() => togglePlatform("facebook")} className={cn("flex min-h-11 items-center gap-2 rounded-xl border px-3 text-sm font-semibold transition", selectedPlatforms.includes("facebook") ? "border-blue-400 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600 hover:bg-slate-50")}><Facebook className="h-4 w-4" /> Facebook{selectedPlatforms.includes("facebook") && <Check className="ml-auto h-4 w-4" />}</button>
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button type="button" onClick={() => setPostType("image")} className={cn("flex min-h-11 items-center gap-2 rounded-xl border px-3 text-xs font-semibold transition sm:text-sm", postType === "image" ? "border-violet-400 bg-violet-50 text-violet-700" : "border-slate-200 text-slate-600 hover:bg-slate-50")}><ImageIcon className="h-4 w-4" /> Image only{postType === "image" && <Check className="ml-auto h-4 w-4" />}</button>
                      <button type="button" onClick={() => setPostType("withLink")} className={cn("flex min-h-11 items-center gap-2 rounded-xl border px-3 text-xs font-semibold transition sm:text-sm", postType === "withLink" ? "border-violet-400 bg-violet-50 text-violet-700" : "border-slate-200 text-slate-600 hover:bg-slate-50")}><Link className="h-4 w-4" /> With link{postType === "withLink" && <Check className="ml-auto h-4 w-4" />}</button>
                    </div>
                  </div>
                  <div className="mt-4 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <label className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Caption</label>
                      <Button type="button" variant="ghost" size="sm" onClick={generateSocialCaption} disabled={!selectedProduct || generatingCaption} className="h-8 gap-1.5 rounded-lg px-2 text-xs text-violet-700">{generatingCaption ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}Generate with AI</Button>
                    </div>
                    <Textarea value={socialCaption} onChange={(event) => setSocialCaption(event.target.value)} placeholder="Write the social caption or generate it with AI..." rows={5} className="resize-none rounded-xl border-slate-200" />
                  </div>
                </section>
              </div>
            </div>
            ...
            <div className="mx-auto max-w-6xl space-y-4 sm:space-y-5">
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
                <section className="grid min-h-[320px] place-items-center overflow-hidden rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:min-h-[460px] sm:p-5">
                  {generating ? (
                    <div className="flex flex-col items-center gap-3 text-center"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-violet-50 text-violet-700"><Loader2 className="h-7 w-7 animate-spin" /></span><div><p className="font-bold text-slate-950">Generating your creative</p><p className="mt-1 text-sm text-slate-500">Keeping your product as the visual reference…</p></div></div>
                  ) : generatedImage ? (
                    <img src={generatedImage} alt="Generated creative" className="max-h-[68vh] w-full rounded-xl object-contain" />
                  ) : (
                    <div className="max-w-sm text-center"><AlertCircle className="mx-auto h-8 w-8 text-amber-500" /><p className="mt-3 font-bold text-slate-950">No preview available</p><p className="mt-1 text-sm text-slate-500">Generation did not complete. Go back to Creative setup and try again.</p></div>
                  )}
                </section>
                <aside>
                  <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Creative summary</p>
                    <dl className="mt-3 space-y-3 text-sm"><div><dt className="text-xs text-slate-400">Template</dt><dd className="mt-0.5 font-semibold text-slate-900">{selectedStyle?.name}</dd></div><div><dt className="text-xs text-slate-400">Product</dt><dd className="mt-0.5 line-clamp-2 font-semibold text-slate-900">{selectedProduct?.title}</dd></div><div><dt className="text-xs text-slate-400">Format</dt><dd className="mt-0.5 font-semibold text-slate-900">{selectedStyle?.size || "square"}</dd></div><div><dt className="text-xs text-slate-400">Direction</dt><dd className="mt-0.5 font-semibold capitalize text-slate-900">{generationMode}</dd></div></dl>
                    <Button type="button" variant="outline" className="mt-4 w-full gap-2" onClick={() => void generateCreative()} disabled={generating}>{generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}Regenerate</Button>
                  </section>
                </aside>
              </div>
            </div>
          ...
          {step === 4 && (
            <div className="mx-auto max-w-4xl space-y-5">
              <div className="text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-emerald-100 text-emerald-700"><Check className="h-7 w-7" /></span><p className="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-violet-600">Step 4</p><h2 className="mt-1 text-2xl font-bold text-slate-950 sm:text-3xl">Validate your creative</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-500">Your creative is ready. Download it now or publish it to the selected social channels.</p></div>
        ...
        <div className="grid shrink-0 grid-cols-2 gap-2 border-t border-slate-200 bg-white px-3 py-3 sm:flex sm:items-center sm:justify-between sm:px-6 sm:py-4">
          ...
          {step < 4 ? (
            <Button type="button" onClick={() => void goNext()} disabled={(step === 1 && !selectedProduct) || generating || (step === 3 && !generatedImage)} className="h-11 rounded-xl bg-violet-600 px-4 hover:bg-violet-700 sm:min-w-40">
