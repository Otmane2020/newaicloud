...
export function MinimalAltWorkspace() {
  ...
  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE)); const rows = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE); const missing = items.filter((item) => !optimized(item)).length;
  const generate = async (targets: AltItem[]) => { if (!targets.length) return; try { setBusy(true); let success = 0; for (const item of targets) { const { data, error } = await supabase.functions.invoke("smart-alt-text", { body: { image_id: item.id, imageType: item.imageType, force: true } }); if (!error && data?.success !== false) success += 1; } toast.success(fr ? `${success} ALT généré(s)` : `${success} ALT text(s) generated`); setSelected(new Set()); await load(); } finally { setBusy(false); } };
