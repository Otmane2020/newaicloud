...
export default function RegenerateLanding({
  ...
  // 🛡️ Guards to prevent infinite loops
  ...
  useEffect(() => {
    ...
    // Auto-generate only once when autoGenerate is true
    if (autoGenerate && !hasAutoGenerated.current && !loading) {
      console.log("🚀 Auto-generating landing page...", { timestamp: new Date().toISOString() });
      hasAutoGenerated.current = true;
      handleGenerate();
    }

    // Reset flag when component unmounts or autoGenerate changes
    return () => {
      if (!autoGenerate) {
   ...
   * 🏷️ Resolve Vendor based on config
   -----------------------------*/
  const resolveVendor = useCallback(async (): Promise<string> => {
  ...
  }, [config.vendorSource, product.id, product.title, product.description]);

  /** ----------------------------
   * 🖼️ Analyze Image with AI Vision
   -----------------------------*/
  const analyzeImageWithAI = useCallback(async (imageUrl: string): Promise<string> => {
    if (!imageUrl) {
  ...
  }, [product.title, config.vendorSource, t.landingGeneration.analyzing]);

  /** ----------------------------
   * 📏 Calculate Content Length Parameters
   -----------------------------*/
  const getContentLengthParams = useCallback(() => {
  ...
  const handleGenerate = useCallback(async () => {
    // 🛡️ Guard against multiple simultaneous calls
    if (isGeneratingRef.current) {
      console.log("⚠️ Generation already in progress, skipping...", { timestamp: new Date().toISOString() });
      return;
    }

    isGeneratingRef.current = true;
    console.log("🚀 Starting generation...", { timestamp: new Date().toISOString() });
    ...
    try {
      ...
      console.log("✅ Generation completed", { timestamp: new Date().toISOString() });
    }
  }, [
    product.id,
    product.title,
