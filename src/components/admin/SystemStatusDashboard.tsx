...
export function SystemStatusDashboard() {
  ...
  const statusColor = !summary ? 'gray' : allHealthy ? 'green' : summary.status === 'degraded' ? 'orange' : 'red';
                  ...
                  <CollapsibleTrigger className="flex items-center justify-between w-full p-4 hover:bg-accent rounded-lg transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium">
                        {t.systemStatus.category[category.toLowerCase().replace(/ /g, '')] || category}
                      </span>
                      {!allCategoryHealthy && (
                        <Badge variant="destructive">{unhealthyCount} {t.systemStatus.issues}</Badge>
                      )}
                    </div>
                      ...
                      {functions.map((func) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4 text-sm">
                            ...
                            {func.error && (
                              <span className="text-red-500 text-xs max-w-xs truncate">{func.error}</span>
                            )}
                          </div>
                        </div>
                      ))}
