...
const PricingComparison = () => {
  ...
  const allPlans = [
    ...
    },
    {
      ...
      name: "Enterprise 128K",
      priceMonthly: 12736,
      priceYearly: 122265.6,
      products: 128000,
      optimizations: 128000,
      articles: 6400,
      chat: 128000,
      stores: 320,
      campaigns: 640,
    ...
    },
    {
      ...
      name: "Enterprise 200K",
      priceMonthly: 19900,
      priceYearly: 191040,
      products: 200000,
      optimizations: 200000,
      articles: 10000,
      chat: 200000,
      stores: 512,
      campaigns: 1000,
  ...
  const features = [
    ...
    { name: "Campagnes éditoriales", key: "campaigns", format: (v: number) => v.toLocaleString() },
    {
      name: "Support",
      key: "support",
      format: (v: string) => (v === "email" ? "Email" : v === "priority" ? "Prioritaire" : "Dédié"),
    },
            ...
            <Card key={plan.id} className="overflow-hidden">
              <CardHeader className="bg-muted/50 pb-3">
                <div className="flex items-center justify-between">
                  <Badge variant={isEnterprise ? "default" : isPro ? "secondary" : "outline"}>{plan.name}</Badge>
                  <span className="text-sm font-bold">{plan.priceMonthly.toFixed(2)}€/mois</span>
                </div>
              </CardHeader>
          ...
          <tbody>
            {features.map((feature, index) => (
              <tr key={index} className="border-b hover:bg-muted/20 transition-colors">
                <td className="p-3 text-sm font-medium sticky left-0 bg-background z-10">{feature.name}</td>
                {allPlans.map((plan) => {
                  ...
                  return (
                    <td key={`${plan.id}-${feature.key}`} className="p-3 text-center text-sm">
                      {typeof value === "boolean" ? (
                        ...
                        <span
                          className={
                            feature.key === "priceMonthly" || feature.key === "priceYearly" ? "font-semibold" : ""
                          }
                        >
                          {displayValue}
                        </span>
