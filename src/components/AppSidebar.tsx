...
export function AppSidebar() {
  ...
  const isChatActive = currentPath.startsWith('/chat') || currentPath === '/product-source' || chatSubItems.some(item => isActive(item.url));
                          ...
                          <CollapsibleContent>
                            <div className="ml-4 border-l border-border pl-2 space-y-1 mt-1">
                              {blogOpportunitiesSubItems.map((item) => (
                                <SidebarMenuSubButton key={item.title} asChild isActive={isActive(item.url)} className="text-xs">
                                  <NavLink to={item.url}>
